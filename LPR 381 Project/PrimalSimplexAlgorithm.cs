...
{
    internal class PrimalSimplexAlgorithm
    {
        ...
        public PrimalSimplexAlgorithm()
        {

        }

        public PrimalSimplexAlgorithm(decimal[][] programTable)
        {
            this.program = programTable;
        }

        public PrimalSimplexAlgorithm(decimal[][] program, string[] restrictions, string[] stSigns, bool maximize, int VariableCount, bool URS)
        {
        ...
        public PrimalSimplexAlgorithm readProgramFromRTB(string text)
        {
            ...
            PrimalSimplexAlgorithm init = new PrimalSimplexAlgorithm(newProgram, restrictionRow, signs, maximize, decVarsCount, URS);
            return init;
        }

        #endregion
        #region Convert To Primal Structure

        public PrimalSimplexAlgorithm ConvertToPrimal(LinearProgramModel model)
        {
            bool isMax = model.IsMaximization;
            int primalVarCount = model.ObjectiveCoefficients.Count + 1; // Adjust as needed

            // Convert objective coefficients from double to decimal
            decimal[] ArrayHold = new decimal[model.ObjectiveCoefficients.Count];
            for (int i = 0; i < model.ObjectiveCoefficients.Count; i++)
            {
                ArrayHold[i] = (decimal)model.ObjectiveCoefficients[i];
            }
            //set obj = 0
            ArrayHold[model.ObjectiveCoefficients.Count - 1] = 0;
            // Initialize algorithm table
            decimal[][] algoTable = new decimal[model.Constraints.Count+1][];
            algoTable[0] = ArrayHold;

            // Populate algoTable with constraint coefficients and RHS
            int constraintNumber = 1;
            foreach (Constraint constraint in model.Constraints)
            {
                decimal[] coefficients = new decimal[model.ObjectiveCoefficients.Count];
                for (int i = 0; i < constraint.Coefficients.Count; i++)
                {
                    coefficients[i] = (decimal)constraint.Coefficients[i];
                }
                coefficients[model.ObjectiveCoefficients.Count-1] = (decimal)constraint.RightHandSide;
                algoTable[constraintNumber] = coefficients;
                constraintNumber++;
            }

            // Prepare restrictions and signs arrays
            string[] restrictions = new string[model.Constraints.Count];
            for (int i = 0; i < model.Constraints.Count; i++)
            {
                restrictions[i] = model.Constraints[i].Relation;
            }

            string[] sTsigns = new string[model.ObjectiveCoefficients.Count];
            for (int i = 0; i < model.ObjectiveCoefficients.Count; i++)
            {
                sTsigns[i] = model.SignRestrictions[i];
            }
         

            // Create PrimalSimplexAlgorithm instance
            PrimalSimplexAlgorithm newAlgo = new PrimalSimplexAlgorithm(algoTable, sTsigns, restrictions, isMax, primalVarCount, false);
        ...
        #endregion
        #region Basic Pivot
        public PrimalSimplexAlgorithm Pivot(int pivotRow, int pivotColumn, PrimalSimplexAlgorithm lp)
        {
        ...
        public int GetPrimalSimplexOptimalStateBasic(PrimalSimplexAlgorithm lp)
        {
        ...
        public PrimalSimplexAlgorithm GetPrimalSimplexCanonical(PrimalSimplexAlgorithm lp)
        {
            decimal[] tempRHS = new decimal[lp.program.Length];
            decimal[][] tempNonRHS = new decimal[lp.program.Length][];
            PrimalSimplexAlgorithm canonicalProgram = new PrimalSimplexAlgorithm();
        ...
        public int GetPrimalSimplexStateNonURSINT(PrimalSimplexAlgorithm lp)
        {
        ...
        public int[] GetPivotRowAndColumnPrimalSimplex(PrimalSimplexAlgorithm lp)
        {
